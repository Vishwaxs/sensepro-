...
**For:** Vishwas (lead) + 2 teammates · **Goal of Phase 1:** repo + environments live, the board-camera capture→recognition→presence loop proven end-to-end, the enrollment quality-gap fixed, the API contract frozen, and the first "overpowered" UI shell standing. **No model training — enrollment only.**
