...
const DEFAULT_SECTION = import.meta.env.VITE_CLASS_SECTION || "MCA-4B";
const DEFAULT_SUBJECT = import.meta.env.VITE_CLASS_SUBJECT || "";
...
const MODES: { id: Mode; label: string; icon: typeof Camera; hint: string }[] = [
  ...
  { id: "exam", label: "Exam", icon: ShieldAlert, hint: "Candidate monitoring + review queue" },
  {
    id: "workshop",
    label: "Workshop",
    icon: Presentation,
    hint: "Anonymous class engagement + coverage",
  },
];
...
function StartSessionPage() {
  ...
  const titleLabel = isExam ? "Examination name" : isWorkshop ? "Workshop name" : "Subject / topic";
  const titlePlaceholder = isExam
    ? "e.g. Mid-Term — Distributed Systems"
    : isWorkshop
      ? "e.g. Cloud Deployment Lab"
      : "e.g. Distributed Systems";
  const LaunchIcon = isExam ? ShieldAlert : isWorkshop ? Presentation : Camera;
  const launchLabel = busy
    ? "Creating…"
    : isExam
      ? "Open exam monitor"
      : isWorkshop
        ? "Open engagement view"
        : "Open camera & begin";
  const processingNote = isExam
    ? "Live frames are analysed in memory; only events sent for review are retained."
    : isWorkshop
      ? "Live frames are analysed in memory; only anonymous class aggregates are retained."
  ...
  async function launch() {
    ...
    setBusy(true);
    const subject = title.trim() || (isExam ? "Examination" : isWorkshop ? "Workshop" : "Lecture");
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (mode !== "lecture") {
          toast.error(
            `${isExam ? "Exam monitoring" : "Workshop engagement"} was not started because its session record could not be created (${res.status})${detail ? `: ${detail}` : ""}`,
          );
          setBusy(false);
          return;
        }
        toast.error(`Session could not be persisted (${res.status})${detail ? `: ${detail}` : ""}`);
      }
    } catch {
      if (mode !== "lecture") {
        toast.error(
          `${isExam ? "Exam monitoring" : "Workshop engagement"} needs a connected backend so its ${isExam ? "review events" : "class aggregates"} can be retained. Nothing was started.`,
        );
        setBusy(false);
        return;
      }
      /* Lecture preserves its live-camera fallback when persistence is offline. */
    }
    const params = new URLSearchParams({ mode, title: subject, section: section.trim() });
    if (room.trim()) params.set("room", room.trim());
    if (sessionId) params.set("session_id", sessionId);
    if (!sessionId) toast.info("Session record unavailable — capture will retry when it opens.");
    ...
    <div className="mx-auto max-w-3xl">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-[color:var(--muted)]">
          Name the lecture, examination, or workshop and pick its operating mode. The camera opens
          on the next screen.
        </p>
      </div>
            ...
            {MODES.map((m) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <div className="font-display text-sm font-bold text-[color:var(--ink)]">
                    {m.label}
                  </div>
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          ...
          <div className="flex items-end">
            <div className="w-full rounded-lg border border-dashed border-[color:var(--line)] bg-[color:var(--surface-2)]/50 px-4 py-3 font-mono-nums text-[11px] leading-relaxed text-[color:var(--muted)]">
              {isExam
                ? "Exam monitoring sends device, additional-person, and sustained head-pose events to a teacher review queue. Nothing is auto-penalised."
                : isWorkshop
                  ? "Workshop engagement is retained only as class-level zone aggregates. No participant receives an engagement score."
