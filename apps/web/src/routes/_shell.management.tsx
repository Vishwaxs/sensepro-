...
function ManagementPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* BiasChart — naive mean vs VNEI (PRD Week 3) */}
      <section className="glass-panel p-6">
        <header>
          <div className="font-mono-nums text-[11px] uppercase tracking-[0.2em] text-[color:var(--muted)]">
            Engagement fairness · bias check
          </div>
          <div className="mt-0.5 font-display text-xl font-extrabold tracking-tight text-[color:var(--ink)]">
            Naive mean vs VNEI
          </div>
          <div className="mt-1 font-mono-nums text-[11px] text-[color:var(--muted)]">
            VNEI corrects for visibility bias. If bars differ, naive mean is misleading.
          </div>
        </header>
        <div className="mt-4">
          <BiasChart zones={biasZones.filter((z) => !z.suppressed)} />
        </div>
      </section>
